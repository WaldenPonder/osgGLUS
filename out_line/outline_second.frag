...
uniform sampler2D baseTexture; //需要边缘检测的纹理
uniform sampler2D baseTexture_no_edge_detection; //不需要边缘检测的纹理
...
void main()
{	
    vec4 c = texture( baseTexture_no_edge_detection, texcoord);

    float dist = sobel_filter();
    gl_FragColor =  vec4(u_color * dist, 1.0) + c; 
