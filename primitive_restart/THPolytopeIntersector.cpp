...
inline bool is_equal_negative_one(const osg::Vec3& p1)
{
	return fabs(p1.x() + 1) < 1e-4 && fabs(p1.y() + 1) < 1e-4 && fabs(p1.z() + 1) < 1e-4;
}
...
namespace THPolytopeIntersectorUtils
{
	...
	{
		Settings() : _polytopeIntersector(0),
					 _iv(0),
					 _drawable(0),
					 _limitOneIntersection(false),
					 _primitiveMask(THPolytopeIntersector::ALL_PRIMITIVES) {}

		THPolytopeIntersector*		  _polytopeIntersector;
	...
	struct IntersectFunctor
	{
		...
		IntersectFunctor() : _primitiveIndex(0),
							 _hit(false)
		{
			src.reserve(10);
			dest.reserve(10);
		...
		void addIntersection()
		{
			...
			THPolytopeIntersector::Intersection intersection;
			...
			if (src.size() < THPolytopeIntersector::Intersection::MaxNumIntesectionPoints)
				intersection.numIntersectionPoints = src.size();
			else
				intersection.numIntersectionPoints = THPolytopeIntersector::Intersection::MaxNumIntesectionPoints;
		...
		void operator()(const osg::Vec3& v0, bool /*treatVertexDataAsTemporary*/)
		{
			...
			if ((_settings->_primitiveMask & THPolytopeIntersector::POINT_PRIMITIVES) == 0)
			{
				++_primitiveIndex;
				return;
			}
		...
		void operator()(const osg::Vec3& v0, const osg::Vec3& v1, bool /*treatVertexDataAsTemporary*/)
		{
			...
			if ((_settings->_primitiveMask & THPolytopeIntersector::LINE_PRIMITIVES) == 0)
			{
				++_primitiveIndex;
				return;
			}
		...
		void operator()(const osg::Vec3& v0, const osg::Vec3& v1, const osg::Vec3& v2, bool /*treatVertexDataAsTemporary*/)
		{
			...
			if ((_settings->_primitiveMask & THPolytopeIntersector::TRIANGLE_PRIMITIVES) == 0)
			{
				++_primitiveIndex;
				return;
			}
		...
		void intersect(const osg::Vec3Array* vertices, int primitiveIndex, unsigned int p0)
		{
			...
			if ((_settings->_primitiveMask & THPolytopeIntersector::POINT_PRIMITIVES) == 0) return;
		...
		void intersect(const osg::Vec3Array* vertices, int primitiveIndex, unsigned int p0, unsigned int p1)
		{
			...
			if ((_settings->_primitiveMask & THPolytopeIntersector::LINE_PRIMITIVES) == 0) return;
		...
		void intersect(const osg::Vec3Array* vertices, int primitiveIndex, unsigned int p0, unsigned int p1, unsigned int p2)
		{
			...
			if ((_settings->_primitiveMask & THPolytopeIntersector::TRIANGLE_PRIMITIVES) == 0) return;
...
//  PolytopeIntersector
//
THPolytopeIntersector::THPolytopeIntersector(const osg::Polytope& polytope) : _parent(0),
																			  _polytope(polytope),
...
THPolytopeIntersector::THPolytopeIntersector(CoordinateFrame cf, const osg::Polytope& polytope) : Intersector(cf),
																								  _parent(0),
																								  _polytope(polytope),
																								  _primitiveMask(ALL_PRIMITIVES)
{
...
THPolytopeIntersector::THPolytopeIntersector(CoordinateFrame cf, double xMin, double yMin, double xMax, double yMax) : Intersector(cf),
																													   _parent(0),
																													   _primitiveMask(ALL_PRIMITIVES)
{
	double zNear = 0.0;
...
osgUtil::Intersector* THPolytopeIntersector::clone(osgUtil::IntersectionVisitor& iv)
{
	if (_coordinateFrame == MODEL && iv.getModelMatrix() == 0)
	{
		osg::ref_ptr<THPolytopeIntersector> pi = new THPolytopeIntersector(_polytope);
		pi->_parent							   = this;
	...
	osg::ref_ptr<THPolytopeIntersector> pi = new THPolytopeIntersector(transformedPolytope);
	pi->_parent							   = this;
	pi->_intersectionLimit				   = this->_intersectionLimit;
	pi->_primitiveMask					   = this->_primitiveMask;
	pi->_referencePlane					   = this->_referencePlane;
...
bool THPolytopeIntersector::enter(const osg::Node& node)
{
	if (reachedLimit()) return false;
	return !node.isCullingActive() || _polytope.contains(node.getBound());
}

void THPolytopeIntersector::leave()
{
	// do nothing.
}

void THPolytopeIntersector::intersect(osgUtil::IntersectionVisitor& iv, osg::Drawable* drawable)
{
	...
	{
		osg::TemplatePrimitiveFunctor<THPolytopeIntersectorUtils::IntersectFunctor<osg::Vec3d>> intersector;
	...
	{
		osg::TemplatePrimitiveFunctor<THPolytopeIntersectorUtils::IntersectFunctor<osg::Vec3f>> intersector;
		intersector._settings = settings;
...
void THPolytopeIntersector::reset()
{
	Intersector::reset();
