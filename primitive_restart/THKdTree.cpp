...
struct BuildKdTree
{
    ...
    bool build(THKdTree::BuildOptions& options, osg::Geometry* geometry);

    void computeDivisions(THKdTree::BuildOptions& options);

    int divide(THKdTree::BuildOptions& options, osg::BoundingBox& bb, int nodeIndex, unsigned int level);
...
bool BuildKdTree::build(THKdTree::BuildOptions& options, osg::Geometry* geometry)
{
    ...
    THKdTree::KdNode node(-1, _primitiveIndices.size());
    node.bb = _bb;
	...
	THKdTree::Indices& primitiveIndices = _kdTree.getPrimitiveIndices();

    THKdTree::Indices new_indices;
...
void BuildKdTree::computeDivisions(THKdTree::BuildOptions& options)
{
...
int BuildKdTree::divide(THKdTree::BuildOptions& options, osg::BoundingBox& bb, int nodeIndex, unsigned int level)
{
    THKdTree::KdNode& node = _kdTree.getNode(nodeIndex);
    ...
    if (node.first<0)
    {
...

        {
            ...
            THKdTree::KdNode leftLeaf(-istart-1, (right-istart)+1);
            THKdTree::KdNode rightLeaf(-left-1, (iend-left)+1);
        ...
        if (!insitueDivision)
        {
            ...
            THKdTree::KdNode& newNodeRef = _kdTree.getNode(nodeIndex);
...
THKdTree::BuildOptions::BuildOptions():
...
THKdTree::THKdTree() : _degenerateCount(0)
{
}

THKdTree::THKdTree(const THKdTree& rhs, const osg::CopyOp& copyop):
    Shape(rhs, copyop),
...
bool THKdTree::build(BuildOptions& options, osg::Geometry* geometry)
{
...
//
// KdTreeBuilder
THKdTreeBuilder::THKdTreeBuilder():
    osg::NodeVisitor(osg::NodeVisitor::TRAVERSE_ALL_CHILDREN)
{
    _kdTreePrototype = new THKdTree;
}

THKdTreeBuilder::THKdTreeBuilder(const THKdTreeBuilder& rhs):
    osg::Object(rhs),
    osg::NodeVisitor(osg::NodeVisitor::TRAVERSE_ALL_CHILDREN),
...
void THKdTreeBuilder::apply(osg::Geometry& geometry)
{
   THKdTree* previous = dynamic_cast<THKdTree*>(geometry.getShape());
    if (previous) return;

    osg::ref_ptr<THKdTree> kdTree = osg::clone(_kdTreePrototype.get());
