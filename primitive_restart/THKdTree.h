...
class THKdTree : public osg::Shape
{
 public:
	THKdTree();

	THKdTree(const THKdTree& rhs, const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY);

	virtual osg::Object* cloneType() const { return new THKdTree(); }
	virtual osg::Object* clone(const osg::CopyOp& copyop) const { return new THKdTree(*this, copyop); }
	virtual bool		 isSameKindAs(const osg::Object* obj) const { return dynamic_cast<const THKdTree*>(obj) != NULL; }
	virtual const char*  libraryName() const { return ""; }
	virtual const char*  className() const { return "THKdTree"; }
	...
	struct KdNode
	{
		...
		KdNode(value_type f, value_type s) : first(f),
											 second(s) {}
...
class THKdTreeBuilder : public osg::NodeVisitor
{
 public:
	THKdTreeBuilder();

	THKdTreeBuilder(const THKdTreeBuilder& rhs);

	META_NodeVisitor(osg, THKdTreeBuilder)

		virtual THKdTreeBuilder* clone()
	{
		return new THKdTreeBuilder(*this);
	}

	void apply(osg::Geometry& geometry);

	THKdTree::BuildOptions _buildOptions;

	osg::ref_ptr<THKdTree> _kdTreePrototype;

 protected:
	virtual ~THKdTreeBuilder() {}
};
